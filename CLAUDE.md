...
| `fortify-development` | `.claude/skills/fortify-development/SKILL.md` | Working on authentication — login, registration, password reset, 2FA, passkeys, profile updates, or any auth-related routes and controllers |
| `inertia-react-development` | `.claude/skills/inertia-react-development/SKILL.md` | Creating React pages, forms, or navigation with Inertia.js; using `<Link>`, `<Form>`, `useForm`, `useHttp`, or router |
| `laravel-permission-development` | `.claude/skills/laravel-permission-development/SKILL.md` | Working with roles, permissions, middleware, policies, teams, or Blade directives using spatie/laravel-permission |
| `pest-testing` | `.claude/skills/pest-testing/SKILL.md` | Writing, editing, fixing, or refactoring Pest PHP tests; converting PHPUnit to Pest; TDD workflows |
| `tailwindcss-development` | `.claude/skills/tailwindcss-development/SKILL.md` | Writing or fixing Tailwind utility classes in templates (Blade, JSX, Vue); responsive layouts, UI components, dark mode |
| `wayfinder-development` | `.claude/skills/wayfinder-development/SKILL.md` | Frontend code needs to call backend routes or controller actions; connecting React/Inertia frontend to Laravel routes |
